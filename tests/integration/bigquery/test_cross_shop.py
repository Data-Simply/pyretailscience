...
def test_cross_shop_with_bigquery(transactions_table, group_3_col):
    ...
    group_1_vals = transactions[group_1_col].execute().dropna().unique()
    group_2_vals = transactions[group_2_col].execute().dropna().unique()
    ...
    if group_3_col is not None:
        group_3_vals = transactions[group_3_col].execute().dropna().unique()
