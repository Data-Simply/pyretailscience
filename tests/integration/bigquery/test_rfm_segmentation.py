...
@pytest.mark.parametrize(
    ("current_date", "limit_rows"),
    [
        (None, 5000),
        ("2023-12-31", 3000),
        (datetime.date(2023, 6, 30), 2000),
    ],
)
def test_rfm_segmentation_with_bigquery(
    ...
    and process data from BigQuery using different current_date parameters and data sizes
    without throwing exceptions.
    """
    limited_table = transactions_table.limit(limit_rows)
