...
class TestCompositeRank:
    ...
    def test_with_custom_column_names(self):
        """Test CompositeRank with completely custom column names."""
        df = pd.DataFrame(
            {
                "custom_product": [1, 2, 3, 4, 5],
                "custom_revenue": [100, 150, 75, 200, 125],
                "custom_buyers": [20, 30, 15, 40, 25],
                "custom_basket_size": [5.0] * 5,
            },
        )
        ...
        assert result_df["composite_rank"].notna().all(), "Composite rank should not contain NaN values"
        assert len(result_df) == expected_length, "Result should have same number of rows as input"

        for rank_col in ["custom_revenue_rank", "custom_buyers_rank", "custom_basket_size_rank"]:
            ranks = result_df[rank_col]
            assert ranks.min() >= expected_rank, f"{rank_col} should have minimum rank of 1"
            assert ranks.max() <= len(df), f"{rank_col} should have maximum rank <= number of rows"

        product_4_row = result_df[result_df["custom_product"] == target_product_id]
        assert len(product_4_row) == expected_rank, "Should find exactly one row for product 4"

        assert product_4_row["custom_revenue_rank"].iloc[0] == expected_rank, "Product 4 should have rank 1 for revenue"
        assert product_4_row["custom_buyers_rank"].iloc[0] == expected_rank, "Product 4 should have rank 1 for buyers"
