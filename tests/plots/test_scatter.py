...
def test_plot_labels_all_nan_no_textalloc_called(sample_product_dataframe, mocker):
    """Test that textalloc is not called when all labels are NaN (empty texts list)."""
    mock_textalloc = mocker.patch("pyretailscience.plots.scatter.ta.allocate")

    # Create a dataframe where all labels are NaN
    df_all_nan_labels = sample_product_dataframe.copy()
    df_all_nan_labels["product_name"] = None  # All labels are NaN

    result_ax = scatter.plot(
        df=df_all_nan_labels,
        value_col="units_sold",
        x_col="price",
        label_col="product_name",
        title="Product Performance with All NaN Labels",
    )

    # Verify basic plot structure
    assert isinstance(result_ax, Axes)
    assert result_ax.get_title() == "Product Performance with All NaN Labels"

    # Verify scatter plot was created (even though no labels)
    collections = [child for child in result_ax.get_children() if hasattr(child, "get_offsets")]
    assert len(collections) >= 1, "No scatter plot collections found"

    # textalloc should not have been called because texts list is empty after NaN filtering
    mock_textalloc.assert_not_called()


def test_plot_labels_with_nan_in_x_col(sample_product_dataframe, mocker):
    """Test that rows with NaN in x_col are properly filtered out from labels."""
    mock_textalloc = mocker.patch("pyretailscience.plots.scatter.ta.allocate")

    # Add NaN to x_col (price) but keep label_col valid
    df_with_nan_x = sample_product_dataframe.copy()
    df_with_nan_x.loc[1, "price"] = None  # x_col has NaN
    ...
    result_ax = scatter.plot(
        ...
        x_col="price",
        label_col="product_name",
        title="Product Performance with NaN in X Column",
    )

    # Verify basic plot structure
    assert isinstance(result_ax, Axes)
    assert result_ax.get_title() == "Product Performance with NaN in X Column"

    # Verify scatter plot was created
    collections = [child for child in result_ax.get_children() if hasattr(child, "get_offsets")]
    assert len(collections) >= 1, "No scatter plot collections found"

    # textalloc should be called with filtered data
    mock_textalloc.assert_called_once()
    args, kwargs = mock_textalloc.call_args
    x_coords = args[1]
    y_coords = args[2]
    ...
    # Should have 4 labels instead of 5 (one row filtered due to NaN in x_col)
    expected_count = len(sample_product_dataframe) - 1  # 5 - 1 = 4
    assert len(labels) == expected_count, (
        f"Expected {expected_count} labels after filtering NaN x_col, got {len(labels)}"
    )

    # Verify all coordinates have valid (non-NaN) values
    for i, (x, y) in enumerate(zip(x_coords, y_coords, strict=False)):
        assert not pd.isna(x), f"Label {i} has NaN x-coordinate: {x}"
        assert not pd.isna(y), f"Label {i} has NaN y-coordinate: {y}"

    # Verify the remaining labels are correct (excluding the filtered row)
    expected_labels = set(df_with_nan_x.dropna(subset=["price", "units_sold", "product_name"])["product_name"].tolist())
    actual_labels = set(labels)
    assert actual_labels == expected_labels, (
        f"Labels don't match after filtering: expected {expected_labels}, got {actual_labels}"
    )


def test_plot_labels_with_empty_label_kwargs(sample_product_dataframe, mocker):
    """Test that empty label_kwargs dict is handled correctly."""
    mock_textalloc = mocker.patch("pyretailscience.plots.scatter.ta.allocate")
    ...
    result_ax = scatter.plot(
        ...
        x_col="price",
        label_col="product_name",
        label_kwargs={},  # Empty dict instead of None
        title="Product Performance with Empty Label Kwargs",
    )

    # Verify basic plot structure
    assert isinstance(result_ax, Axes)
    assert result_ax.get_title() == "Product Performance with Empty Label Kwargs"

    # Verify scatter plot was created
    collections = [child for child in result_ax.get_children() if hasattr(child, "get_offsets")]
    assert len(collections) >= 1, "No scatter plot collections found"

    # textalloc should be called with empty kwargs dict
    mock_textalloc.assert_called_once()
    args, kwargs = mock_textalloc.call_args
    labels = args[3]
    ...
    # Verify that default textalloc kwargs are present even when label_kwargs is empty
    # Should include default parameters from our implementation
    assert "textsize" in kwargs, "textsize should be in kwargs"
    assert "x_scatter" in kwargs, "x_scatter should be in kwargs"
    assert "y_scatter" in kwargs, "y_scatter should be in kwargs"
    assert "nbr_candidates" in kwargs, "nbr_candidates should be in kwargs"
    assert kwargs["nbr_candidates"] == NBR_CANDIDATES_DEFAULT, (
        f"Expected nbr_candidates={NBR_CANDIDATES_DEFAULT}, got {kwargs['nbr_candidates']}"
    )


def test_plot_labels_default_no_lines(sample_product_dataframe, mocker):
    """Test that draw_lines=False is set by default to avoid connecting lines."""
    mock_textalloc = mocker.patch("pyretailscience.plots.scatter.ta.allocate")
    ...
    result_ax = scatter.plot(
        ...
        x_col="price",
        label_col="product_name",
        title="Product Performance Default No Lines",
    )

    # Verify basic plot structure
    assert isinstance(result_ax, Axes)
    assert result_ax.get_title() == "Product Performance Default No Lines"

    # Verify scatter plot was created
    collections = [child for child in result_ax.get_children() if hasattr(child, "get_offsets")]
    assert len(collections) >= 1, "No scatter plot collections found"

    # textalloc should be called with draw_lines=False by default
    mock_textalloc.assert_called_once()
    args, kwargs = mock_textalloc.call_args
    labels = args[3]
    ...
    # Verify that draw_lines=False is set by default
    assert "draw_lines" in kwargs, "draw_lines should be in kwargs"
    assert kwargs["draw_lines"] is False, f"Expected draw_lines=False, got {kwargs['draw_lines']}"


def test_plot_labels_can_override_draw_lines(sample_product_dataframe, mocker):
    """Test that users can override draw_lines=True if they want connecting lines."""
    mock_textalloc = mocker.patch("pyretailscience.plots.scatter.ta.allocate")
    ...
    result_ax = scatter.plot(
        ...
        label_col="product_name",
        label_kwargs=custom_kwargs,
        title="Product Performance with Lines Override",
    )

    # Verify basic plot structure
    assert isinstance(result_ax, Axes)
    assert result_ax.get_title() == "Product Performance with Lines Override"

    # Verify scatter plot was created
    collections = [child for child in result_ax.get_children() if hasattr(child, "get_offsets")]
    assert len(collections) >= 1, "No scatter plot collections found"

    # textalloc should be called with user-provided draw_lines=True
    mock_textalloc.assert_called_once()
    args, kwargs = mock_textalloc.call_args
    labels = args[3]
    ...
    # Verify that user's draw_lines=True override works
    assert "draw_lines" in kwargs, "draw_lines should be in kwargs"
    assert kwargs["draw_lines"] is True, f"Expected draw_lines=True (override), got {kwargs['draw_lines']}"
    assert "linecolor" in kwargs, "linecolor should be in kwargs"
    assert kwargs["linecolor"] == "red", f"Expected linecolor='red', got {kwargs['linecolor']}"
