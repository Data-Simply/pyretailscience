...
def _add_point_labels(
    ...
    if group_col is None:
        ...
        for x_val, y_val, label in zip(
            x_values,
            data_with_labels_df[value_col],
            data_with_labels_df[label_col],
            strict=False,
        ):
            all_x_coords.append(x_val)
            all_y_coords.append(y_val)
            all_labels.append(str(label))
    else:
        # Multi-series with groups - need to match labels to correct groups
